...
public class Box2DUtils {
	...
	public static final Box2DComponent create(Sprite sprite, BodyType bodyType) {
		...
		shape.setAsBox(Pixels.toMeters(sprite.getScaleX() * sprite.getWidth() / 2),
				Pixels.toMeters(sprite.getScaleY() * sprite.getHeight() / 2));
	...
	public static final Box2DComponent create(float x, float y, int width, int height, BodyType bodyType) {
		...
		Body body = createBody(x, y, width, height, bodyType);

		PolygonShape shape = new PolygonShape();
		shape.setAsBox(Pixels.toMeters(width / 2),
				Pixels.toMeters(height / 2));
